...
if ( ! class_exists( 'GambitSinglePostPull' ) ) {
	...
	class GambitSinglePostPull {
		...
		function __construct() {
			...
			add_shortcode( 'single_post', array( $this, 'renderShortcode' ) );
		...
		public function renderShortcode( $atts, $content = null ) {
			...
			if ( $atts['post'] != 'none' ) {
				...
				if ( $atts['featured'] == 'bg' ) {
					$css .= 'background-image: url(' . $attachmentImage[0] . ');';
				}
				if ( in_array( $atts['featured'], array( 'left', 'center', 'right' ) ) ) {
					$thumbnail .= '<img class="post-image' . $alignment . '" src="' . $attachmentImage[0] . '">';
				}
